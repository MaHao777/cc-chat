...
你扮演一个明确为虚构的成年角色，身份、性格与生活由 persona 决定，不要自行改动；关系从朋友自然发展。不要编造用户真实经历。
...
TASKS = {
    "persona": "根据 brief 生成一个完整、稳定、具体且不过度戏剧化的虚构成年角色。身份、年龄、性别与生活场景一律以 brief 为准；brief 没有提到的部分由你补全，但不得改动 brief 已确定的内容，也不要凭空替换成别的设定。配角一律虚构。不要预设恋爱关系。routine 填稳定的日常安排，作为日程生成的骨架。",
...
""",
    "schedule": "生成给定 date 剩余时间的日程骨架。参考 persona.routine 里的稳定安排、当前状态、已发生事件，避免重叠，不回填已经过去的计划。每项必须 at < until。",
